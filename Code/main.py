...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if UI.mode == 'AI vs AI':
                if QuanCo.luot == 't':
                    if UI.may1 == 'Stockfish':
                        if (ai_thread1 is None or not ai_thread1.is_alive()):
                            ai_thread1 = threading.Thread(target=AIChoiThread, args=(
                            stockfish, banco_matrix, QuanCo.luot, UI.may1, ai_result_queue1))
                            ai_thread1.start()
                    else:
                        if (ai_thread1 is None or not ai_thread1.is_alive()):
                            ai_thread1 = threading.Thread(target=AIChoiThread, args=(
                            None, banco_matrix, QuanCo.luot, UI.may1, ai_result_queue1))
                            ai_thread1.start()
                else:
                    if UI.may2 == 'Stockfish':
                        if (ai_thread2 is None or not ai_thread2.is_alive()):
                            ai_thread2 = threading.Thread(target=AIChoiThread, args=(
                            stockfish, banco_matrix, QuanCo.luot, UI.may2, ai_result_queue2))
                            ai_thread2.start()
                    else:
                        if (ai_thread2 is None or not ai_thread2.is_alive()):
                            ai_thread2 = threading.Thread(target=AIChoiThread, args=(
                            None, banco_matrix, QuanCo.luot, UI.may2, ai_result_queue2))
                            ai_thread2.start()
            if UI.mode == 'P vs AI':
                if QuanCo.luot == 't':
                    QuanCo.DiChuyenCo(banco_matrix, event)
                ...
                if UI.may == 'Stockfish':
                    if QuanCo.luot == 'd' and (ai_thread2 is None or not ai_thread2.is_alive()):
                        ai_thread2 = threading.Thread(target=AIChoiThread, args=(
                        stockfish, banco_matrix, QuanCo.luot, UI.may, ai_result_queue2))
                        ai_thread2.start()
                else:
                    if QuanCo.luot == 'd' and (ai_thread2 is None or not ai_thread2.is_alive()):
                        ai_thread2 = threading.Thread(target=AIChoiThread,
                                                      args=(None, banco_matrix, QuanCo.luot, UI.may, ai_result_queue2))
                        ai_thread2.start()
